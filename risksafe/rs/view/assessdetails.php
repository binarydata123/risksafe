...
<div class="row">
  ...
  <div class="col-lg-9 col-md-12">
    <div class="panel panel-default">
      ...
      <div class="alert alert-info" id="notify" <?php if (!isset($msg)) { ?>style="display: none;"<?php } ?>>
         ...
         <div class="form-group">
            ...
            <textarea name="descript" rows="4" class="form-control" placeholder="Enter risk description..." required><?php if ($edit) echo $data["as_descript"];?></textarea>
    ...
    <div class="form-group">
            ...
            <?php 
			  echo $assess->listControlsLib($_SESSION["userid"]);
			?>
            </select>
...
$(document).ready(function(e) {
 ...
 <?php
 	...
 	if ($edit) {
		...
		echo 'selrisk=' . $data["as_risk"] . ';';
		echo 'selhazard=' . $data["as_hazard"] . ';';
		echo 'cathazard=' . $data["as_risk"] . ';';
 ...
 $(function() {
  ...
  $("#btn_addtreatment").click(function(e) {
  	if ($("#treatment").val()!=='') {
		$.ajax({type: "GET", url: "../controller/assessment.php?action=addtreat&descript="+$("#treatment").val()+'&id='+detid, async: false})
		$("#treatments").load('../controller/assessment.php?action=listtreat&id='+detid);
		$("#treatment").val("");	
	}
  });
  ...
  $("#btn_addcontrol").click(function(e) {
  	if ($("#control").val()!=='') {
		$.ajax({type: "GET", url: "../controller/assessment.php?action=addcontrol&descript="+$("#control").val()+'&id='+detid, async: false})
		$("#controls").load('../controller/assessment.php?action=listcontrols&id='+detid);
		$("#control").val("");	
	}
  });
   ...
   $("#existing_ct").change(function(e) {
  	if ($("#exisitng_ct").val()!=='-1') {
		$.ajax({type: "GET", url: "../controller/assessment.php?action=addlibcontrol&id="+$("#existing_ct").val()+'&det='+detid, async: false})
		$("#controls").load('../controller/assessment.php?action=listcontrols&id='+detid);
		$("#control").val("");
		$("#existing_ct").val("-1");	
	}
   ...
   $("#existing_tr").change(function(e) {
  	if ($("#exisitng_tr").val()!=='-1') {
		$.ajax({type: "GET", url: "../controller/assessment.php?action=addlibtreat&id="+$("#existing_tr").val()+'&det='+detid, async: false})
		$("#treatments").load('../controller/assessment.php?action=listtreat&id='+detid);
		$("#treatment").val("");
		$("#existing_tr").val("-1");	
	}
...
function del(what, id) {
	...
	if (what=="treatment") {
		$.ajax({type: "GET", url: "../controller/assessment.php?action=deletetreat&id="+id, async: false})
		$("#treatments").load('../controller/assessment.php?action=listtreat&id='+detid);
	}
	
	if (what=="control") {
		$.ajax({type: "GET", url: "../controller/assessment.php?action=deletecontrol&id="+id, async: false})
		$("#controls").load('../controller/assessment.php?action=listcontrols&id='+detid);
	}
