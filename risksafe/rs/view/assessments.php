...
if (isset($_REQUEST["action"]) && $_REQUEST["action"]=="downloadxls") {
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$row, $assessment['ty_name'])
	            ...
	            ->setCellValue('E'.$row, $assessment['idassessment'])
	...
	foreach ($list as $item) {
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$row, $item['ri_name'])
	            ->setCellValue('B'.$row, $item['cat_name'])
	            ->setCellValue('C'.$row, $item['as_descript'])
	            ->setCellValue('D'.$row, $item['li_like'])
				->setCellValue('E'.$row, $item['con_consequence'])
				->setCellValue('F'.$row, $as->getRating($item['as_rating']))
				->setCellValue('G'.$row, $as->listControlsForReport($item['iddetail']))
				->setCellValue('H'.$row, $item['as_effect'])
				->setCellValue('I'.$row, $item['ac_type'])
				->setCellValue('J'.$row, $as->listTreatmentsForReport($item['iddetail'])) // treat
    ...
    <div class="panel panel-default">
      <div class="panel-body">
          ...
          <div class="clearfix mb20">
          	<button type="button" class="btn btn-md btn-info pull-right" id="btn_add">+ New Risk Assessment</button>
          </div>
