<?php
	...
	if (isset($_REQUEST["action"]) && $_REQUEST["action"]=="listdetails") {
		...
		foreach ($list as $item) {
			...
			<a title="View Risk Details" class="btn btn-xs btn-info" href="assessdetails.php?action=editdetail&assessmentId='.$_REQUEST["id"].'&id=' . $item["iddetail"] . '"><i class=" glyphicon glyphicon-eye-open"></i></a>&nbsp;
	...
	if (isset($_REQUEST["action"]) && $_REQUEST["action"]=="listassess") {
		...
		foreach ($list as $item) {
			...
			<a target="_blank" title="Download XLS" class="btn btn-warning btn-xs" href="assessments.php?action=downloadxls&id=' . $item["idassessment"]. '"><i class=" glyphicon glyphicon-download"></i></a></div>';
			$data[] = array_values($response);
		}
